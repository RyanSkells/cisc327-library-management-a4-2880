...
def get_patron_borrowed_books(patron_id: str) -> List[Dict]:
    ...
    # This for loop has been modified to include the borrow_record ID column in the borrowed_books list
    # This change simplifies the calculation for late fees when returning a book
    borrowed_books = []
...
def insert_book(title: str, author: str, isbn: str, total_copies: int, available_copies: int) -> bool:
    """Insert a new book into the database. If there is an existing book with the same isbn, increase the # of copies"""
    conn = get_db_connection()
    dupes = conn.execute('''
    ...
    try:
        if dupes:
            dupe = {'id': dupes[0], 'total_copies': dupes[4], 'available_copies': dupes[5]}
            conn.execute('''
                UPDATE books SET total_copies = total_copies + ? WHERE id = ?
            ''', (available_copies, dupes['id']))
            conn.commit()
            update_book_availability(dupes['id'], available_copies)
        else:
            conn.execute('''
                INSERT INTO books (title, author, isbn, total_copies, available_copies)
                VALUES (?, ?, ?, ?, ?)
...
def update_borrow_record_return_date(record_id: int, return_date: datetime) -> bool:
    """Update the return date for a borrow record."""
    #  NOTE: This was originally updating ALL records with the matching book and patron IDs.
    #  I have altered it to only change the record with the correct record ID within the database.
    #  This will keep the book return function consistent.
    conn = get_db_connection()
    try:
        conn.execute('''
...
def get_patron_borrow_records(patron_id: str) -> List[Dict]:
    """Fetch a patron's borrowing history"""
    conn = get_db_connection()
    records = conn.execute('''
            SELECT * FROM borrow_records where patron_id = ? 
            ORDER BY borrow_date
        ''', (patron_id, )).fetchall()
    conn.close()

    # Different append operations are present in case a book is still checked out and thus has a null value in its
    # return_date entry
    history = []
    if records:
        for record in records:
