...
def add_book_to_catalog(title: str, author: str, isbn: str, total_copies: int) -> Tuple[bool, str]:
    ...
    # This check has been commented out in order to support adding more copies of preexisting books
    """
...
def borrow_book_by_patron(patron_id: str, book_id: int) -> Tuple[bool, str]:
    ...
    # Note: this condition was > 5 previously, allowing a user to check out 6 books total.
...
def return_book_by_patron(patron_id: str, book_id: int) -> Tuple[bool, float]:
    ...
    # Check if book exists and has been borrowed by the user
    book = get_book_by_id(book_id)
    if not book:
        return False, "Book not found."
    ...
    if not borrowed_books:
        return False, "User has no books checked out."

    #grab first borrow_record with matching patron & book ID
    record_id = 0
    overdue = False
    ...
    if not record_id:
        return False, "User has not checked that book out."

    # If book is valid and has been checked out by the user, update the available copies.
    update_book_availability(book_id, 1)

    update_borrow_record_return_date(record_id, datetime.now())
...
def calculate_late_fee_for_book(patron_id: str, book_id: int) -> Dict:
    ...
    """
    fee_details = {'fee_amount' : 0, 'days_overdue' : 0, 'status' : ""}

    # Select correct record information
    due_book = {}
    borrowed_books = get_patron_borrowed_books(patron_id)
    for idx, book in enumerate(borrowed_books):
        if book_id == book['book_id']:
            due_book = {'due_date': book['due_date']}
            break

    # Perform fee calculation
    fee_details['days_overdue'] = (datetime.now() - due_book['due_date']).days
    if 7 >= fee_details['days_overdue'] >= 0:
        fee_details['fee_amount'] = fee_details['days_overdue'] * 0.5
    if fee_details['days_overdue'] > 7:
        fee_details['fee_amount'] = (7 * 0.5) + (fee_details['days_overdue'] - 7)
    # Apply fee cap
    if fee_details['fee_amount'] > 15:
        fee_details['fee_amount'] = 15

    fee_details['status'] = True
...
def search_books_in_catalog(search_term: str, search_type: str) -> List[Dict]:
    ...
    if search_type == "title":
        for idx, book in enumerate(get_all_books()):
    ...
    if search_type == "isbn":
        for idx, book in enumerate(get_all_books()):
            if search_term.lower() == book['isbn'].lower():
                search_list.append(book)
...
def get_patron_status_report(patron_id: str) -> (List[Dict], float, int, List[Dict]):
    """
    ...
    # go through all borrow records, pick out ones with matching patron ID
